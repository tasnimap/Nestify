...
public sealed class HelperWorkspaceService
{
    ...
    {
        using var connection = await _db.OpenAsync();
        var row = await connection.QuerySingleOrDefaultAsync<NavRow>("""
            ...
            FROM users u
            LEFT JOIN domestic_helper_profiles hp ON hp.user_id = u.id
            WHERE u.id = @userId
            """, new { userId, requested = Requested });

        return row is null
            ? new HelperNavDto()
            : new HelperNavDto
            {
                HasProfile = row.PhotoUrl is not null,
                Name = row.Name,
    ...
    public async Task<(HelperProfileDto? Data, string? Error)> RegisterAsync(long userId, HelperProfileFormDto form)
    {
        ...
        if (exists)
        {
            return (null, "You already have a helper profile.");
        }
        ...
        var helperId = await connection.ExecuteScalarAsync<long>("""
            INSERT INTO domestic_helper_profiles (user_id, headline, bio, languages, years_experience, monthly_rate)
            VALUES (@userId, @headline, @bio, @languages, @years, @rate)
            RETURNING id
            """, new
        {
            userId,
        ...
        // A sensible first board: every day except Friday, 8 AM to 6 PM. She
        // can change it on the availability page.
    ...
    public async Task<(HelperProfileDto? Data, string? Error)> UpdateProfileAsync(long userId, HelperProfileFormDto form)
    {
        ...
        if (helperId is null)
        {
            return (null, "Create your helper profile first.");
        }
        ...
        await ReplaceServicesAsync(connection, transaction, helperId.Value, form.Services);
    ...
    private static string? Validate(HelperProfileFormDto form)
    {
        ...
        if (form.Services.Count == 0) return "Pick at least one service you offer.";
        if (form.MonthlyRate <= 0) return "Set a monthly rate.";
        if (form.ExperienceYears is < 0 or > 60) return "Years of experience looks wrong.";
        if (form.UpazilaId is null) return "Pick the area where you live.";
        if (string.IsNullOrWhiteSpace(form.AddressLine)) return "Write your address.";
        if (form.Latitude is < -90 or > 90 || form.Longitude is < -180 or > 180) return "Drop the pin on the map where you live.";
        if (form.Headline.Length > 120) return "Keep the headline under 120 characters.";
        if (form.Bio.Length > 1000) return "Keep the bio under 1000 characters.";
        if (form.Languages.Length > 120) return "Keep the languages under 120 characters.";
        return null;
    }
    ...
    {
        using var connection = await _db.OpenAsync();
        var helperId = await GetHelperIdAsync(connection, userId);
        if (helperId is null)
        {
            return null;
        }
        ...
        var summary = await connection.QuerySingleAsync<DashboardRow>("""
            ...
            GROUP BY hp.average_rating, hp.review_count, hp.is_verified
    ...
    public async Task<string?> SaveAvailabilityAsync(long userId, HelperAvailabilityDto dto)
    {
        ...
        if (helperId is null)
        {
            return "Create your helper profile first.";
        }
    ...
    public async Task<string?> DecideAsync(long userId, string engagementId, bool accept, string? reason)
    {
        ...
        var changed = await connection.ExecuteAsync(accept
                ...
                : """
                  ...
                  WHERE id = @id AND helper_profile_id = @helperId AND status = @requested
                  """,
            new { id, helperId, requested = Requested, active = Active, declined = Declined, reason });

        return changed == 0 ? "This request was already answered." : null;
    }
    ...
    private static Task<long?> GetHelperIdAsync(IDbConnection connection, long userId) =>
        connection.ExecuteScalarAsync<long?>("SELECT id FROM domestic_helper_profiles WHERE user_id = @userId", new { userId });
