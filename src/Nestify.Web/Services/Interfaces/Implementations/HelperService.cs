...
public sealed class HelperService : IHelperService
{
        ...
        HelperFilterDto filter)
    {
        ...
        var result =
            await _http.GetFromJsonAsync<HelperPageDto<HelperSummaryDto>>(
                $"api/v1/helpers{query}");
    ...
    {
        return await _http.GetFromJsonAsync<HelperDetailDto>(
            $"api/v1/helpers/{id}");
    }
    ...
    public async Task<HelperDetailDto> RegisterAsync(
    ...
    public async Task<HelperDetailDto> UpdateProfileAsync(
        HelperRegistrationDto dto)
    {
    ...
    public async Task<HelperPageDto<ReviewDto>> GetReviewsAsync(
        string helperId,
        int page = 1,
        int pageSize = 5)
    {
        var result =
            await _http.GetFromJsonAsync<HelperPageDto<ReviewDto>>(
    ...
    {
        var result =
            await _http.GetFromJsonAsync<List<EngagementDto>>(
                "api/v1/helpers/engagements");

        return result ?? new List<EngagementDto>();
    }

    public async Task<EngagementDto> RequestEngagementAsync(
        string helperId,
        IReadOnlyList<EngagementSlotDto>? slots = null)
    {
        var response = await _http.PostAsJsonAsync(
            ...
            new
            {
                Slots = slots ?? Array.Empty<EngagementSlotDto>()
            });
    ...
    public async Task<EngagementDto> ConfirmEngagementAsync(
        string engagementId)
    {
        var response = await _http.PostAsync(
            $"api/v1/helpers/engagements/{engagementId}/confirm",
            null);
    ...
    public async Task<EngagementDto> MarkCompleteAsync(
        string engagementId)
    {
        var response = await _http.PostAsync(
            $"api/v1/helpers/engagements/{engagementId}/complete",
            null);
        ...
        IBrowserFile file)
    {
        ...
        var response = await _http.PostAsync(
            "api/v1/helpers/me/verification",
            form);
